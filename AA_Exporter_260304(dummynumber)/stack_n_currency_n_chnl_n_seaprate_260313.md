...
- 처리 대상 파일 목록에서 union_*.csv 제외
  → 이전: union_ 파일이 포함되어 매핑 없음 skip_list에 불필요하게 출력됨
  → 이후: f.name.startswith("union_") 필터 추가로 처음부터 제외
- channel 후처리 시 value 컬럼 dtype 보정 추가 (버그픽스)
  → 원인: _latest_map으로 선택된 channel CSV의 value 컬럼이 int64인 경우,
          map(us_mapping).fillna(...) 결과(object dtype)를 대입 시 TypeError 발생
  → 수정: channel 루프 시작 시 df_long["value"] = df_long["value"].astype(object) 추가
- non-channel 파일에도 site_code별 dummy 0 삽입 로직 추가
  → 이전: channel 파일에만 dummy 0 삽입, non-channel 파일은 추출된 사이트만 포함
  → 이후: 메인 루프 전 pre-scan 단계에서 전체 CSV를 한 번 순회하여
          us_ prefix 파일 → _us_sites, 나머지 → _global_sites로 분리 수집
          각 tb_key 처리 시 해당 그룹 마스터 site_code 기준으로 누락 site×metric_col 조합에 0행 삽입
          (파일에 아예 없던 사이트도 포함: pre-scan 메타 + 파일 대표 날짜로 채움)

...
- ⚠️ 매핑초과(api 추출 된 CSV부족): tb_column_name_mapping의 value_n이 CSV보다 많음 → 매핑은 있는데 CSV에 해당 컬럼 없음
- ⚠️ CSV초과(tb_column_name매핑누락): CSV에 value19~20 이상 등이 있는데 tb_column_name_mapping에 없음 → 매핑 추가 필요
...
| 상태 | 조치 |
|------|------|
| ⚠️ 매핑초과(api 추출 된 CSV부족) | AA 추출 시 해당 value_n 컬럼이 실제로 없는지 확인. 없으면 tb_column_name_mapping에서 해당 행 제거 또는 플레이스홀더 표시 |
| ⚠️ CSV초과(tb_column_name매핑누락) | CSV의 csv_only 컬럼(value19~20 등)을 tb_column_name_mapping에 추가 필요 |

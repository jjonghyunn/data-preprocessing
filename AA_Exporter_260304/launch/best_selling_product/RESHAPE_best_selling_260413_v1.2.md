# RESHAPE_best_selling_260413_v1.2.py 가이드
...
## v1.1 → v1.2 변경사항 (2026-04-28, Jonghyun Park)

### [FIX-3] PRICE RANGE 컬럼 추가 (PRODUCT ↔ ORDER 사이)

- **추가 위치**: 출력 컬럼 순서에서 `CATEGORY` 다음, `ORDER` 직전
- **계산식**: 단가 = `REVENUE / ORDER`
- **5구간 분류**:

  | 단가 (price) | PRICE RANGE |
  |---|---|
  | `price < 300` | `Under $300` |
  | `300 ≤ price < 500` | `Under $500` |
  | `500 ≤ price < 800` | `Under $800` |
  | `800 ≤ price < 1000` | `Under $1000` |
  | `price ≥ 1000` | `Over $1000` |
...
- **구현**:
  ...
  def get_price_range(price: float) -> str:
      ...
      return "Over $1000"
  ```
- **적용 시점**: ORDER=0 제거 후 (division-by-zero 안전)
- **음수 단가**: REVENUE 음수(반품) 행은 단가도 음수 → `Under $300` 으로 분류됨
...
- **효과**: 공란/NaN PRODUCT 행은 모두 `DIVISION="ETC"`, `CATEGORY="ETC"`

### [FIX-2] ORDER == 0 행 결과에서 제거

- **문제**: scom/camp 분리 후 한쪽 ORDER가 0인 행이 그대로 출력
- **수정**: `pd.concat([scom, camp])` 후 `result[result["ORDER"] != 0]` 필터
- **효과**: scom/camp 각각 독립적으로 ORDER=0 행 제거
...
aa_exports/{tb_key}_*.csv
    ...
    ↓
    [v1.2 FIX-3] PRICE RANGE 부여 (단가 = REVENUE/ORDER 기준 5구간)
    ↓
...
```
PERIOD, STANDARD, TIER, SUBS, COUNTRY, SITE CODE, DIVISION, PRODUCT, CATEGORY, PRICE RANGE, ORDER, REVENUE
```
...
| **`PRICE RANGE`** | **Under $300 / Under $500 / Under $800 / Under $1000 / Over $1000 (v1.2 신규)** — 단가 = REVENUE/ORDER 기준 |
...
| **ETC** | **NaN/공란/"NAN" (v1.1)**, LUMAFUSION, ARCSITE, UNSPECIFIED, 기타 미매칭 |
...
| **ETC** | **NaN/공란/"NAN" (v1.1)**, 미매칭 |

**스페인어 displayname fallthrough** (prefix 미매칭 시):  
`MONITOR`, `PRODUCT NAME` (→TV), `FUNDA`/`SOPORTE` (→ACC), `BUDS` (→Wearable), `AIRE ACONDICIONADO` (→AC), `SMART TV` (→TV), `REFRIGERADOR` (→REF), `AURA STUDIO`/`JBL LIVE 770NC`/`TUNE BEAM`/`JBL TOUR ONE` (→AUDIO), `GALAXY WATCH` (→Watch), `IN EAR CORDED EARP` (→Wearable), `메모리카드`/`REMOCON-ECO`/`SOLARCELL REMOTE` (→X)
...
```bash
python "RESHAPE_best_selling_260413_v1.2.py"
```
...
| **PRICE RANGE 음수 단가** | **v1.2부터 추가됨** — 반품/환불로 REVENUE 음수면 단가 음수 → `Under $300` 분류 |

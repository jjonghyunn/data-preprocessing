...
- `extract_panel_tables_json_v2.0.py` (별도 repo: AA-Segments-Maker-by-API) — Workspace project 의 panel × reportlet → JSON + 매핑 CSV 자동 생성 (segments + metric 컬럼 포함)
- `fill_column_by_similarity.py` (별도 repo: AA-Segments-Maker-by-API/column_filler) — 이전 시즌의 column 컬럼 정리본을 reference 로 similarity 기반 채움. 본 generator 와 결합해서 비교 가능 (`FILLED_CSV` 지정 시 자동 비교).

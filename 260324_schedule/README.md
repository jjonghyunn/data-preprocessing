# md_schedule — 캠페인 일정 자동 정제 Excel 도구

> 처음 보는 사람을 위한 구조·수식 가이드
> 작성: 2026-03-24 / 최종 수정: 2026-03-25
> **※ 이 README는 공개용 샘플입니다. 법인명은 CORP-XX 형식으로 익명화, 담당자는 가명 처리.**
...
└── sample_md_schedule.xlsx    ← 생성된 샘플 파일 (Excel/Sheets로 열기)
```
...
| **O** | **Subs** | `=IF(ISBLANK(AD),"",IF(AM="한총","CORP-KR",IF(LEN(TRIM(AM))=0,AB,AM)))` |
...
| AM = "한총" | `"CORP-KR"` | 한국법인 → CORP-KR |
| AM이 비어있음 (매칭 실패) | AB 폴백 | 마스터 표준명 사용 |
| 그 외 | AM 그대로 | `CORP-UK`, `CORP-BN-BE`, `CORP-UK-IE` 등 |
...
| AB | Subsidiary | 표준 법인명 (CORP-UK, CORP-BN, CORP-WA …) |
| AC | Country | 국가명 |
...
1순위: AB + "-" + UPPER(site_code "_" 앞부분) 형태가 AU에 있으면 → dash-variant 키 사용
       예) AB=CORP-BN, site_code=be  → "CORP-BN-BE"  → AU에 있음 → AM=CORP-BN-BE
           AB=CORP-UK, site_code=ie  → "CORP-UK-IE"  → AU에 있음 → AM=CORP-UK-IE  ★site 우선
           AB=CORP-AS, site_code=sg  → "CORP-AS-SG"  → AM=CORP-AS-SG

1.5순위 ★: AB + " (PROPER(prefix)_UPPER(suffix))" 형태가 AU에 있으면 → 괄호-variant 키 사용
       site_code에 "_"가 있을 때, "_" 기준으로 앞부분은 PROPER, 뒷부분은 UPPER로 조합
       예) AB=CORP-WA, site_code=africa_en → "CORP-WA (Africa_EN)" → AU에 있음 → AM=CORP-WA (Africa_EN)
           AB=CORP-WA, site_code=africa_fr → "CORP-WA (Africa_FR)" → AU에 있음 → AM=CORP-WA (Africa_FR)
           AB=CORP-WA, site_code=africa_pt → "CORP-WA (Africa_PT)" → AU에 없음 → 다음 순위로
       * site_code에 "_"가 없으면 COUNTIF 오류 → 0으로 처리되어 안전하게 통과

2순위: AB 값이 AU에 그대로 있으면 → AB 사용
       예) AB=CORP-UK → AU에 CORP-UK 있음 → AM=CORP-UK
...
2.5순위 ★: site_code에 "_"가 포함된 경우 → 즉시 "" 반환 (3순위 이하 스킵)
       ...
       예) AB=CORP-WA, site_code=africa_pt → 1.5/2순위 실패 → AM="" → O=AB="CORP-WA"
           → AU에 plain "CORP-WA" 없음 → AN="" → 최종 출력 미포함

3순위: AL(AR_clean)로 AU에서 MATCH 후 원본 반환  ← site_code에 "_" 없는 사이트에만 적용
       예) AB=CORP-SA → AL="CORP-SA" → AU에서 "CORP-SA (ZA)" 검색 → AM=CORP-SA (ZA)

4순위: AB 자체에 "-" 포함된 경우 → AB 앞부분만으로 AU 검색
       예) AB=CORP-EG-S → LEFT(AB, "-" 앞) = "CORP-EG" → AU에 있음 → AM=CORP-EG

5순위: AB = "CORP-CA"인 경우 → site_code 끝 3자리로 Africa 구분  (현재 사실상 미사용)
       예) site_code=africa_fr → "_fr" → AM=CORP-WA (Africa_FR)
           site_code=africa_en → "_en" → AM=CORP-WA (Africa_EN)
       ※ CORP-WA Africa 사이트는 현재 AB="CORP-WA"로 관리되어 1.5순위에서 처리됨.
```

> **1.5순위와 2.5순위는 쌍으로 동작합니다.**
> 괄호-variant(CORP-WA Africa_EN/FR)는 1.5순위에서, 매칭 실패한 지역 변형(africa_pt)은 2.5순위에서 처리합니다.
...
```excel
=IF((AW="X")+(AW=""), "",
    IF(COUNTIF($O$4:$O$191, IF(AU="한총","CORP-KR",AU)) > 0, "", "사용 안됨")
)
```
...
|---|-----|-----------|---------|----------|
| 1 | EU | CORP-UK | United Kingdom | uk |
| 2 | EU | CORP-UK | Ireland | ie |
| 3 | EU | CORP-BN | Belgium | be |
| 4 | AF | CORP-WA | Africa | africa_en |
| 5 | AF | CORP-WA | Africa | africa_fr |
| 6 | AF | CORP-WA | Africa | africa_pt |
| 7 | AP | 한총 | Korea | kr |
| 8 | AF | CORP-SA | South Africa | za |
...
|--------|------|-----|--------------|-----------|---------|
| EU Region | Total | | | | |
| | CORP-UK | 홍길동 | O | 2026-04-01 | 2026-05-31 |
| | CORP-UK-IE | 김철수 | O | 2026-04-15 | 2026-05-31 |
| | CORP-BN-BE | 이영희 | X | | |
| AF Region | Total | | | | |
| | CORP-WA (Africa_EN) | 박민준 | O | 2026-04-01 | 2026-05-31 |
| | CORP-WA (Africa_FR) | 최수진 | X | | |
| AP Region | Total | | | | |
...
africa_pt (고객파일 미등록) / CORP-SA (미회신)은 제외됩니다.

| RHQ | Subs | Country | RS | site_code | start_date | end_date |
|-----|------|---------|-----|-----------|-----------|---------|
| EU | CORP-UK | United Kingdom | United Kingdom | uk | 2026-04-01 | 2026-05-31 |
| EU | CORP-UK-IE | Ireland | Ireland | ie | 2026-04-15 | 2026-05-31 |
| EU | CORP-BN-BE | Belgium | Belgium | be | X | |
| AF | CORP-WA (Africa_EN) | Africa | Africa (en) | africa_en | 2026-04-01 | 2026-05-31 |
| AF | CORP-WA (Africa_FR) | Africa | Africa (fr) | africa_fr | X | |
| AP | CORP-KR | Korea | Korea | kr | 2026-04-08 | 2026-05-31 |
...
| 케이스 | 현상 | 해결 |
|--------|------|------|
| 한총 (한국법인) | AU = "한총" → AM 매칭 → O열에서 CORP-KR 강제 변환 | 자동 처리됨 |
| CORP-EG-S (이집트 등) | AB="CORP-EG-S", AU="CORP-EG" — 이름 불일치 | AM 4순위 자동 처리 |
| CORP-UK-IE (아일랜드) | AB="CORP-UK", AU="CORP-UK-IE" — base 이름이 먼저 매칭될 위험 | AM 1순위 우선 처리 |
| CORP-WA Africa_EN/FR | AU="CORP-WA (Africa_EN/FR)" — AL 정제 시 둘 다 "CORP-WA"로 동일해져 3순위 오매칭 | AM 1.5순위 PROPER/UPPER 패턴 직접 조회 |
| CORP-WA Africa_PT | 고객파일 미등록 → AM="" → O=AB="CORP-WA" → AU에 없음 → 최종 출력 제외 | AM 2.5순위 자동 처리 |

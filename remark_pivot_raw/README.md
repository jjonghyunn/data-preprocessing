...
```
# 0. 마스킹 시드 지정 (필수 — 미설정이면 스크립트가 안내 후 중단)
$env:REMARK_SEED = "<숫자>"
...
- ⚠️ **시드도 역추적 키다.** cipher 는 시드만 있으면 그대로 재현되므로, `REMARK_SEED` 값을 코드·문서·공유 파일에 남기지 말 것. 세 스크립트에 **같은 값**을 주면 서로 매핑이 일치하고, 값을 바꾸면 이전 산출물과 매핑이 달라진다.
...
- **시드 고정**: 동일 입력이면 언제나 동일 출력 (재현 가능). 시드는 `REMARK_SEED` 환경변수로 주입한다 — 아래 참조
